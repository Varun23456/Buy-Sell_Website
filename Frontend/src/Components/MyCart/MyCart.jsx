...
function MyCart() {
    ...
    useEffect(() => {
        ...
        const getmycartitems = async () => {
            try {
                const response = await axios.post("/api/getMycartItems", { message: "getitems" }, {
                    withCredentials: true,
                });
                if (response.status === 201) {
    ...
    const handleRemoveItem = async (myitem) => {
        ...
        const removefromcart = async () => {
            try {
                const response = await axios.post("/api/removefromcart", { myitem }, {
                    withCredentials: true,
                });
    ...
    const handlePlaceOrder = async () => {
        ...
        const placeorder = async () => {
            try {
                const response = await axios.post("/api/placeorder", { Mycart }, {
                    withCredentials: true,
                });
                if (response.status === 201) {
                    ...
                    const clearcartitems = async () => {
                        try {
                            const response = await axios.post("/api/clearcartItems", { message: "getitems" }, {
                                withCredentials: true,
                            });
                            if (response.status === 201) {
